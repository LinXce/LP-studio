...
function fakePty() {
  ...
  const spawns: { file: string; args: string[]; cwd: string; env: NodeJS.ProcessEnv }[] = [];
  const module: PtyModule = {
    spawn(file, args, options) {
      spawns.push({ file, args, cwd: options.cwd, env: options.env });
      return {
...
test('terminal spawns the CLI in the project and forwards bytes both ways', async t => {
  ...
  if (process.platform === 'win32') {
    assert.equal(pty.spawns[0].env.TERM, 'dumb');
    assert.equal(pty.spawns[0].env.FORCE_COLOR, '3');
    assert.equal(pty.spawns[0].env.NO_COLOR, undefined);
  }
