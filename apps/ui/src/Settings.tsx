...
export function Settings({ theme, setTheme, dataDir, hasProject, relocate }: Props) {
  return <section className="workspace-page settings-page">
    ...
    <div className="page-body">
      ...
      <section className="settings-section"><h3>快捷键</h3><div className="shortcut-list">{[['Ctrl + B', '折叠 / 展开项目栏'], ['Ctrl + J', '折叠 / 展开预览栏'], ['Ctrl + L', '返回输入框'], ['Ctrl + Enter', '运行对话或命令']].map(([key, label]) => <div key={key}><span>{label}</span><kbd>{key}</kbd></div>)}</div></section>
